...
def test_run_is_hashed_and_replayable():
    with TemporaryDirectory() as tmp:
        ...
        run=repo.create_run(RunCreate(strategy_id=strategy.id,strategy_version="1.0.0",dataset_version_id=dataset.id,trades=trades))
...
def test_identical_manifest_is_not_silently_duplicated():
    with TemporaryDirectory() as tmp:
        ...
        req=RunCreate(strategy_id=strategy.id,strategy_version="1.0.0",dataset_version_id=dataset.id,trades=[TradeObservation(pnl_gross=1,fees=0,slippage=0,planned_risk=1,planned_reward=3)])
        repo.create_run(req)
